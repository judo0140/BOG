...
class guide3:
	...
	self.pub = rospy.Publisher('guide3_position', String, queue_size=10)
	self.guide3_position_str = raw_input("Where is the CUP3? (side/center) : ")
	self.dynamixel_position_str = raw_input("Where is the teapot? : ")
	self.table_mode_str = "none"
	rospy.loginfo(self.guide3_position_str)
	self.pub.publish(self.guide3_position_str)
	rospy.sleep(rospy.Duration(0.2))
	print("Send message : The CUP3 is on the %s of table" % (self.guide3_position_str))
    ...
    def callback(self, data):
        ...
        if data.data == "Hierarchical 3" or data.data == "Flat":
	    if self.guide3_position_str == "center":
	        self.reverse()
	        self.guide3_position_str = "side"
	        rospy.loginfo(self.guide3_position_str)
	        self.pub.publish(self.guide3_position_str)
	        rospy.sleep(rospy.Duration(0.2))
	        print("Send message : The CUP3 is on the side of table.")
	    elif self.guide3_position_str == "side":
		print("WRONG COMMAND")
		return
	    self.table_mode_str = data.data
	elif data.data == "Hierarchical 1" or data.data == "Hierarchical 2":
	    if self.guide3_position_str == "center":
	        sleep(4)
	        self.reverse()
	        self.guide3_position_str = "side"
		...
		if self.guide3_position_str == "side":
		    if self.table_mode_str == "Flat" or self.table_mode_str == "none":
	                self.forward()
	                self.guide3_position_str = "center"
	                rospy.loginfo(self.guide3_position_str)
	                self.pub.publish(self.guide3_position_str)
	                rospy.sleep(rospy.Duration(0.2))
	                print("Send message : The CUP3 is on the center of table.")
		    else:
			print("WRONG COMMAND")
		elif self.guide3_position_str == "center":
		    print("WRONG COMMAND")
		    return
	    else:
	    ...
	    if self.dynamixel_position_str == "d0" or  self.dynamixel_position_str == "d6" or self.dynamixel_position_str == "d12":
		if self.guide3_position_str == "side":
		    if self.table_mode_str == "Hierarchical 3" or self.table_mode_str == "none":
			self.forward()
		    elif self.table_mode_str == "Hierarchical 1" or self.table_mode_str == "Hierarchical 2":
		        sleep(4)
	                self.forward()
...
if __name__ == '__main__':
    ...
    rospy.init_node('guide3', anonymous=False)
    guide3()
    rospy.spin()
