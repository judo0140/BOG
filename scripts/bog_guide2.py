...
class guide2:
	...
	self.pub = rospy.Publisher('guide2_position', String, queue_size=10)
	self.guide2_position_str = raw_input("Where is the CUP2? (side/center) : ")
	self.dynamixel_position_str = raw_input("Where is the teapot? : ")
	self.table_mode_str = "none"
	rospy.loginfo(self.guide2_position_str)
	self.pub.publish(self.guide2_position_str)
	rospy.sleep(rospy.Duration(0.2))
	print("Send message : The CUP2 is on the %s of table" % (self.guide2_position_str))
    ...
    def callback(self, data):
        ...
        if data.data == "Hierarchical 2" or data.data == "Flat":
	    if self.guide2_position_str == "center":
	        self.reverse()
	        self.guide2_position_str = "side"
	        rospy.loginfo(self.guide2_position_str)
	        self.pub.publish(self.guide2_position_str)
	        rospy.sleep(rospy.Duration(0.2))
	        print("Send message : The CUP2 is on the side of table.")
	    elif self.guide2_position_str == "side":
		print("WRONG COMMAND")
		return
	    self.table_mode_str = data.data
	elif data.data == "Hierarchical 1" or data.data == "Hierarchical 3":
	    if self.guide2_position_str == "center":
	        sleep(4)
	        self.reverse()
	        self.guide2_position_str = "side"
		...
		if self.guide2_position_str == "side":
		    if self.table_mode_str == "Hierarchical 2" or self.table_mode_str == "none":
			self.forward()
		    elif self.table_mode_str == "Hierarchical 1" or self.table_mode_str == "Hierarchical 3":
		        sleep(4)
		        self.forward()
		    elif self.table_mode_str == "Flat":
			print("WRONG COMMAND")
			return
		    self.guide2_position_str = "center"
		    rospy.loginfo(self.guide2_position_str)
		    self.pub.publish(self.guide2_position_str)
		    rospy.sleep(rospy.Duration(0.2))
		    print("Send message : The CUP2 is on the center of table.")
		elif self.guide2_position_str == "center":
		    print("WRONG COMMAND")
		    return
	    else:
...
if __name__ == '__main__':
    ...
    rospy.init_node('guide2', anonymous=False)
    guide2()
    rospy.spin()
